...
#endif
STATIC_MODIFIER slabclass_t slabclass[POWER_LARGEST + 1];
STATIC_MODIFIER size_t mem_limit = 0;
static size_t mem_malloced = 0;
static int power_largest = -1;
/*
...
#if NEED_DEBUG >= 1
STATIC_MODIFIER uint64_t alloc_times = 0;
STATIC_MODIFIER uint64_t free_times = 0;
STATIC_MODIFIER uint64_t realloc_times = 0;
#endif /* ! NEED_DEBUG */
...
#ifdef __FreeBSD__
    printf("stat: mem_malloced: %zd, alloc_times: %lu, free_times: %lu, realloc_times: %lu\n", mem_malloced, alloc_times, free_times, realloc_times);
#elif linux
#if __x86_64__
    printf("stat: mem_malloced: %zd, alloc_times: %lu, free_times: %lu, realloc_times: %lu\n", mem_malloced, alloc_times, free_times, realloc_times);
#else
    printf("stat: mem_malloced: %zd, alloc_times: %llu, free_times: %llu, realloc_times: %llu\n", mem_malloced, alloc_times, free_times, realloc_times);
#endif /* ! linux */
#else
#error "OS unknown"
#endif /* __FreeBSD__ */
...
#if NEED_DEBUG >= 1
bool
is_valid_mem(void *ptr)
{
    slabhead_t *head;

    head = (slabhead_t *)((intptr_t)ptr - sizeof(slabhead_t));

    return (head->magic_head == 0xfeedbeef);
}
#else
bool
is_valid_mem(void *ptr __attribute__((unused)))
{
    return true;
}
#endif
